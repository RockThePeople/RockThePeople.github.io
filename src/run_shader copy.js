...
const mining = async (inputdifficulty, global_start, dispatchSize, workSize, nonce_per_thread, blockheader, address) => {

  // const difficultyN = Math.ceil(Number(inputdifficulty)/4);
  const dispatchSizeN = Number(dispatchSize);
  const workSizeN = Number(workSize);

  const returnObejct = [];
  const baseHashArray = fromHexString(blockheader + address);
  // const difficulty = returnTarget(inputdifficulty);
  const difficulty = inputdifficulty;
  ...
  const shaderModule = device.createShaderModule({
    code:
      ...
      @group(0) @binding(1) var<storage, read> difficultySize : array<u32>;
      @group(0) @binding(2) var<storage, read> difficulty : array<u32>;
      ...
      fn main(
        ...
        var local_difficultySize : u32 = difficultySize[0];
        var local_input : array<u32, headerLen>;
        var nonce_array : array<u32, 10>;
        let thread_id : u32 = (workgroup_id.x * ${workSizeN}) + local_invocation_index;

        let nonce_start : u32 = ${global_start} + thread_id * ${nonce_per_thread};
        let nonce_end :u32 = nonce_start + ${nonce_per_thread}-1u;

        for (var i : u32 = 0; i < inputLen; i++) { local_input[i] = input[i]; }
        for (var i : u32 = 0; i < SHA256_BLOCK_SIZE; i++) { local_difficulty[i] = difficulty[i]; }
...
export default async function miningResult(difficulty, range_size, dispatch, worksize, blockheader, iterIndex, startingPoint, lastIteration, address) {
  const global_start = startingPoint + range_size * iterIndex;
  if (lastIteration !== 0) {
    const nonce_per_thread = Math.ceil(lastIteration / (worksize * dispatch));
    return await mining(difficulty, global_start, dispatch, worksize, nonce_per_thread, blockheader, address);
  }
  const nonce_per_thread = Math.ceil(range_size / (worksize * dispatch));
  return await mining(difficulty, global_start, dispatch, worksize, nonce_per_thread, blockheader, address);
}