...
function Frontend() {
    ...
    const [deviceInfo, setDeviceInfo] = useState({ device: "none", dispatch: 0, worksize: 0, range_size: 0 });
    ...
    const selectDevice = (value) => {
        const deviceMap = {
            0: { device: "none", dispatch: 0, worksize: 0, range_size: 0 },
            1: { device: "s22", dispatch: 24, worksize: 28, range_size: 500000 },
            2: { device: "m1air", dispatch: 28, worksize: 96, range_size: 5000000 },
            3: { device: "m1pro", dispatch: 24, worksize: 128, range_size: 5000000 },
            4: { device: "m3pro", dispatch: 60, worksize: 96, range_size: 5000000 }
        };
    ...
    let hashs = [];
    const manualMining = async (target, range_size, dispatch, worksize, blockheader, iterate, lastIterationSize, lastIteration = 0) => {
        const res = await miningResult(target, range_size, dispatch, worksize, blockheader, iterate, lastIterationSize, lastIteration, address);
        if (res[1] && res[2]) {
            keepWorking = false;
    ...
    const handleBigMessage = async (message) => {
        ...
        if (message.type === 'bigEpoch' && isMiningEnabled) {
            const nonceRange = message.nonceRange;
            let iterate = Math.floor(nonceRange / deviceInfo.range_size);
            let divided = nonceRange % deviceInfo.range_size;
            epochInfo = { target: message.target, dispatch: message.dispatch, worksize: message.worksize, nonceStart: 0, nonceEnd: message.nonceRange, iterate: iterate, lastIteration: divided };
        }
    ...
    const handleLittleMessage = async (message) => {
        if (message.type === 'littleEpoch' && isMiningEnabled && keepWorking) {
            ...
            for (let i = 0; i < epochInfo.iterate; i++) {
                ...
                const res = await manualMining(epochInfo.target, deviceInfo.range_size, epochInfo.dispatch, epochInfo.worksize, message.header, i, epochInfo.nonceStart);
                if (res[1] && res[2]) {
                    setResult({ hash: res[1], nonce: res[2] })
                    console.log(res[1]);
                    setHash(hashs);
            ...
            const res = await manualMining(epochInfo.target, deviceInfo.range_size, epochInfo.dispatch, epochInfo.worksize, message.header, epochInfo.iterate, epochInfo.nonceStart, epochInfo.lastIteration);
            console.log(`👷‍♂️⛏️ Round : ${epochInfo.iterate + 1}/${epochInfo.iterate + 1}, Work time : ${res[0]}`);
        }
    };
